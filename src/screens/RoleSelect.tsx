    ...
    <div style={{ fontFamily: C.fn, background: C.snow, minHeight: "100vh" }}>
      <div style={{ background: C.white, padding: isMobile ? "24px 16px" : "32px 24px" }}>
        <div style={{ maxWidth: 720, margin: "0 auto" }}>
          ...
          <h2 style={{ fontSize: isMobile ? 20 : 24, fontWeight: 800, color: C.charcoal, marginBottom: 4 }}>Hi, {s.name}.</h2>
          ...
          {ROLES.map(r => {
            ...
            return (
              <div key={r.id}
                ...
                style={{ display: "flex", alignItems: "center", gap: 14, padding: isMobile ? "12px 14px" : "14px 16px", marginBottom: 5, background: sel ? r.bg : C.white, border: `1.5px solid ${sel ? r.color : C.border}`, borderLeft: `4px solid ${r.color}`, cursor: "pointer" }}>
          ...
          {s.roles.length > 0 && (
            <button onClick={() => { u({ phase: "baseline", blIdx: 0, bl: [], blQs: BL.map((v, i) => v[(s.seed + i) % v.length]) }); scrollTop(); }}
              style={{ background: C.red, color: "#fff", border: "none", padding: isMobile ? "13px 20px" : "14px 28px", fontSize: 14, fontWeight: 700, fontFamily: C.fn, cursor: "pointer", width: "100%", marginTop: 16, display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
