...
export default function IntakeShell({
  ...
  return (
    <div style={{ fontFamily: C.fn, background: C.dark, minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
      {/* Top bar */}
      <div style={{ padding: isMobile ? '14px 16px' : '16px 24px', display: 'flex', alignItems: 'center', gap: 12, borderBottom: `1px solid ${C.borderD}` }}>
        <Logo size={24} light />
        <div style={{ flex: 1 }}>
          {/* Progress bar */}
          <div style={{ height: 3, background: C.dark2, borderRadius: 2, overflow: 'hidden' }}>
            <div style={{ height: '100%', width: `${pct}%`, background: C.teal, transition: 'width 0.4s ease', borderRadius: 2 }} />
          </div>
          <div style={{ fontSize: 10, color: C.ash, marginTop: 5, letterSpacing: 1 }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: isMobile ? '28px 16px 120px' : '40px 0 100px' }}>
        <div style={{ maxWidth: 560, margin: '0 auto', padding: isMobile ? '0' : '0 24px' }}>
          <h1 style={{ fontSize: isMobile ? 22 : 26, fontWeight: 800, color: C.white, marginBottom: subtitle ? 8 : 24, lineHeight: 1.3 }}>
            {title}
          </h1>
          {subtitle && (
            <p style={{ fontSize: 14, color: C.ash, lineHeight: 1.7, marginBottom: 28, maxWidth: 460 }}>
              {subtitle}
            </p>
          )}
          {children}
      ...
      <div style={{
        ...
        {onBack && (
          <button onClick={onBack} style={{
            background: 'transparent', border: `1px solid ${C.borderD}`,
            color: C.ash, padding: '13px 20px', fontSize: 13, fontWeight: 700,
        ...
        <button
          ...
          style={{
            flex: 1, background: continueDisabled ? C.dark2 : C.teal,
            color: continueDisabled ? C.ash : C.white,
