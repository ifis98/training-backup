    ...
    <div style={{ fontFamily: C.fn, background: C.white, minHeight: "100vh" }}>
      ...
      <div style={{ background: C.white, padding: "32px 24px" }}>
        <div style={{ maxWidth: 720, margin: "0 auto" }}>
          ...
          <div style={{ fontSize: 16, fontWeight: 700, color: C.charcoal, marginBottom: 18, lineHeight: 1.5 }}>{q.q}</div>
          {q.opts.map((o: string, i: number) => (
            <div key={i} onClick={() => { const b = [...s.bl]; b[s.blIdx] = i; u({ bl: b }); }}
              style={{ padding: "13px 16px", marginBottom: 5, background: sel === i ? C.tealBg : C.snow, border: `1.5px solid ${sel === i ? C.teal : C.border}`, cursor: "pointer", display: "flex", alignItems: "center", gap: 12 }}>
